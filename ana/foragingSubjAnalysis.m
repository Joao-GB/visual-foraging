...
% for passado o número das sessões, dá pra pular as que não interessam
    ...
    if nargin < 4, appendNum = false; end
    if appendNum, outFolder = fullfile(outFolder, [params.matPreffix sprintf('%02d', subj)]); end

    if nargin < 5, doPlots = true; end

    if isempty(allSes)
        allSes = foragingSearchSes(subj, outFolder, params);
    end

    allTrlProps      = struct([]);
    allTrainTrlProps = struct([]);
    for ses=allSes
        ...
        [trlProps, ~, ~] = foragingTrlProps(mat, edf, sesStr, subj);
        
        [allGoodTrl, ~] = getGoodTrl(trlProps, mat, 'TRIALS EXPERIMENTO');
%         auxTrlProps = trlProps;
        trlProps = trlProps(allGoodTrl);
        allTrlProps = [allTrlProps trlProps]; %#ok<*AGROW> 
...
%             trainTrlProps2 = trainTrlProps(trainAllTrl);
            allTrainTrlProps = [allTrainTrlProps trainTrlProps1];
        end
        clear edf;
    end

    clear currFolder outFolder params parentFolder sesStr ses subj trlProps trainTrlProps1;
    ...
    preProbePos        = pixel_to_dva([allTrlProps.preProbePosPix], 'dist', mat.prm.screenDist, 'width', mat.dpP.monitorW_mm/10, 'res', mat.dpP.screenRes.width)';
    preProbePosFix     = pixel_to_dva([allTrlProps.preProbePosFixPix], 'dist', mat.prm.screenDist, 'width', mat.dpP.monitorW_mm/10, 'res', mat.dpP.screenRes.width)';
    probePos      = pixel_to_dva([allTrlProps.probePosPix], 'dist', mat.prm.screenDist, 'width', mat.dpP.monitorW_mm/10, 'res', mat.dpP.screenRes.width)';
    ...
    if ~skipAll
        ...
        elseif strcmp(skipMode, 'none')
            ...
            plotPSAdurations(allTrlProps, mat.drP)
        end
    end
    ...
    if ~skipAll
        ...
        elseif strcmp(skipMode, 'none')
            ...
            PSA = plotPSAmain(allTrlProps, mat.drP);
    ...
    if ~skipAll
        ...
        elseif strcmp(skipMode, 'none')
            ...
            order = plotPSAorder(allTrlProps, mat.drP);
            PSA.order = order;
        end
    end
    ...
    if ~skipAll
        ...
        elseif strcmp(skipMode, 'none')
            ...
            plotPSAforagingPerformance(allTrlProps, PSA, mat.drP);
            
            % Efeito do número de vistos (no pré-s e na duração da fixação)
            plotPSAforagingNumSeen(allTrlProps, mat.drP)
        end
    end
    ...
    if ~skipAll
        ...
        elseif strcmp(skipMode, 'none')
            ...
            plotPSApinkDur(allTrlProps, mat)
        
            % Efeito da duração da fixação no desempenho
            plotPSAfixDur(allTrlProps, mat)
            ...
            plotPSAprobesFixDist(allTrlProps, mat)
        end
    end
    ...
    if ~skipAll
        ...
        elseif strcmp(skipMode, 'none')
            ...
            plotPSAdprimeMap(probePos, probePosFix, nSaccProbePos, allTrlProps, mat)
        
            % Mapa de d-prime para verificar efeito da direção da sacada
            plotPSAdprimeMap(rotProbePos, rotProbeFix, rotNSaccProbePos, allTrlProps, mat, true);
        end
    end
    ...
    if ~skipAll
        ...
        elseif strcmp(skipMode, 'none')
            ...
            taskStimDistr(allTrlProps, mat, 2)
        end
    end
