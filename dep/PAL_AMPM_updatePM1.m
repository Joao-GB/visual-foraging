...
function PM = PAL_AMPM_updatePM1(PM,response,varargin)
...
% --- BEGIN EPSILON-GREEDY EXPLORATION ---
epsilon = 0.1; % 10% chance to explore randomly
if rand(1) < epsilon
    % Pick a random index from the stimulus range
    PM.I = randi(length(PM.stimRange));
else
    % Exploit: pick the intensity that minimizes expected entropy
    [minEntropy, PM.I] = min(squeeze(expectedEntropy));
end
% --- END EPSILON-GREEDY EXPLORATION ---
