function instructForaging(tkP, dpP, drP, txP, prm)
...
%     gotoAtypical = [3 5];
    ...
    screens = { 
        @(x,y,z,w)drawBlocks(tkP, dpP, drP, txP, prm, targetOri, b);                          % Tela 1: blocos 
        @(x,y,z,w)DrawCalibrationTarget(dpP.window, dpP.winCenter(1), dpP.winCenter(2));      % Tela 2: calibração
        @(x,y,z,w)startFix(tkP, dpP, drP, txP, prm, fixCoords, fixCenters);                   % Tela 3: fixação inicial
        @(x,y,z,w)foragingDrawMain(dpP.window, gaborTex, noiseTex, srcRects, ...
        ...
        @(x,y,z,w)foragingDrawPedestal(dpP.window, noiseTex, srcRects, dstRects, orientation, txP);    %@(x,y,z,w)drawPM(tkP, dpP, drP, txP, prm, dstRects, orientation, textColor2);         % Tela 6: pedestais
        ...
        textColor2, stimCenters, w, x, targetOri);                       % Tela 8: pós-resposta
        @(x,y,z,w)obsPink(tkP, dpP, drP, txP, prm, allPinkTex, ...              % Tela 9: diferenças de intensidade
        gaborTex, noiseTex, targetOri)
        @(x,y,z,w)obsArrows(tkP, dpP, drP, txP, prm, leftKey, rightKey, targetKey, ...        % Tela 10: correspondência entre 
        targetOri)                                                                        %          setas e alvo ou distrator
        @(x,y,z,w)startFix1(tkP, dpP, drP, txP, prm, fixCoords, fixCenters);                  % Tela 11
        @(x,y,z,w)warnTimeOut(tkP, dpP, drP, txP, prm, fixCoords, fixCenters, ...             % Tela 12: aviso de tempo  (antecedida por tela 3/11)
        overlayColor, overlayRect, textColor1)  ;
        @(x,y,z,w)foragingDrawMain1(tkP, dpP, drP, txP, prm, gaborTex, noiseTex, ...
        srcRects, dstRects, orientation, [repmat(alphas', [3,1]); ones(1, nStims)]);
        @(x,y,z,w)warnFewSacc(tkP, dpP, drP, txP, prm, dstRects, overlayColor, ...
        overlayRect, textColor1, alphas(1), textColor2);                                         % Tela 14: aviso poucas sacadas (antecidida por tela 4/13)
        @(x,y,z,w)drawPink(tkP, dpP, drP, txP, prm, oriPinkTex, gaborTex, ...
        noiseTex, srcRects, dstRects, z, y, orientation);                          % Tela 16: aviso de que mexeu o olho (antecedida por tela 5/15) 
        @(x,y,z,w)warnMov(tkP, dpP, drP, txP, prm, stimCenters, dstRects, y);
        };
    ...
    fixIdx = []; blinkIdx = []; orderToReportStims = []; prevSelected = -1;
    while ~quit
        Screen('FillRect', dpP.window, drP.grey);

        % Desenha a tela selecionada
        if ismember(selected, [5 13 14 16]) && ~ismember(prevSelected, [5 13 14 16])
            [fixIdx, blinkIdx, orderToReportStims] = updateFixIdx([mx;my], minFixDist3, nStims, stimCenters, fixIdx, blinkIdx, orderToReportStims);
        end
        prevSelected = selected;

        if selected == 7
            [outKey, allTargets] = screens{selected}(dpP.window, fixIdx, blinkIdx, orderToReportStims);
            pressed = true; 
...
function [fixIdx, blinkIdx, orderToReportStims] = updateFixIdx(mCoord, minDist, nStims, stimCenters, oldFixIdx, blinkIdx, orderToReportStims)
    isCurrStim = vecnorm(mCoord - stimCenters) <= minDist;
    fixIdx = find(isCurrStim, 1);

    if ~isequal(fixIdx, oldFixIdx)
        blinkIdx = setdiff(1:nStims, fixIdx);
    
        if isempty(fixIdx)
            nbhd = NeighborsOrder(stimCenters, 1, []);
        else
...
function obsPink(tkP, dpP, drP, txP, prm, allPinkTex, gaborTex, noiseTex, targetOri)
    ...
    xSpacing = imageWidth  * 1.25;
    ySpacing = imageHeight * 1.25;

    % Centers of the leftmost and rightmost columns
    leftX  = round(w * (3/8));
    rightX = round(w * (5/8));
    ...
    % Grid centers
    leftGridCenterX  = leftX;
    rightGridCenterX = rightX;
    gridCenterY      = h / 2;

    % Top-left corner of each grid
    leftGridX0  = leftGridCenterX  - gridWidth  / 2;
    rightGridX0 = rightGridCenterX - gridWidth  / 2;
    ...
    for row = 1:nRows
        for col = 1:nCols
            ...
            % Draw corresponding texture on left and right
    ...
    % Text above the leftmost/rightmost column
    bounds = Screen('TextBounds', dpP.window, leftText);
    textWidth = RectWidth(bounds);

    leftTextX = leftGridCenterX - textWidth / 2;
    leftTextY = gridY0 - 50 - bounds(4) + bounds(2);

    DrawFormattedText(dpP.window, leftText, ...
        leftTextX, leftTextY, drP.black);

    bounds = Screen('TextBounds', dpP.window, rightText);
    textWidth = RectWidth(bounds);

    rightTextX = rightGridCenterX - textWidth / 2;
    rightTextY = gridY0 - 50 - bounds(4) + bounds(2);

    DrawFormattedText(dpP.window, rightText, ...
        rightTextX, rightTextY, drP.black);
...
function warnMov(~, dpP, drP, txP, prm, stimCenters, dstRects, fixIdx)
    if isempty(fixIdx), fixIdx = 1; end
    Screen('TextSize', dpP.window, prm.textSizeEnormous);
    titleText = 'Advertências'; titleMargin = dpP.winCenter(1)*prm.titleMarginFactor;
    Screen('DrawText', dpP.window, titleText, titleMargin, titleMargin,  drP.blackGrey);
