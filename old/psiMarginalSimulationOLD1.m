function [estParams, expectedX, estX, sdX] = psiMarginalSimulationOLD1(marginalizeParams, plotOption, NumTrials, paramsGen, stairLevel)
...
%     paramsGen(1) = expectedX;
    ...
    if doPlot
        alphas = linspace(-80, -20, plotGrain);
        betas = 10.^(linspace(log10(0.005), log10(0.7), plotGrain));
        gammas = gamma;
    ...
    priorBetaRange  = 10.^(linspace(log10(0.005), log10(0.7), 50));
    ...
    % Computa o valor estimado de x85
    c = (stairLevel - gamma) / (1-gamma-lambda);
    z = -sqrt(2)*erfinv(1-2*c);

    x85Grid = PM.priorAlphas + z ./ PM.priorBetas;
    posterior = PM.pdf;
    estX = sum(x85Grid(:).*posterior(:));

    sdX = sqrt(sum(posterior(:).*(x85Grid(:)-estX).^2));
