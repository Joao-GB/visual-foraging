...
%% Cria o objeto que registra todo o staircase
        if tkP.stairBurnIn
            ...
            burnInASigma = logspace(log10(prm.sigmaMin), log10(prm.sigmaMax), burninTrials);
        ...
        alphaRange = (-log10(prm.sigmaMax) : 0.05 : -log10(prm.sigmaMin));
        PF = @PAL_Gumbel;
        ...
        if tkP.stairBurnIn == 0 && isfield(tkP, 'stairPrev') && ~isempty(tkP.stairPrev)
            for b = 1:nBlocks
                ...
                RF(b) = PAL_AMRF_setupRF( ...
                    ...
                    'xMin', -log10(prm.sigmaMax), ...
                    'xMax', -log10(prm.sigmaMin), ...
                    'meanmode', 'mean', ...
                    'stopCriterion', 'trials', ...
                    'stopRule', (nStims-1)*nTrials); %#ok<AGROW> 
            end
        else
            prior = PAL_pdfNormal(alphaRange, -log10(prm.priorMeanStair), prm.priorStdStairLog);
            RF = PAL_AMRF_setupRF( ...
                'priorAlphaRange', alphaRange, ...
                'prior', prior, ...
                'PF', PF, ...
...
%% Atualiza o staircase se o trial foi útil
                        for j=1:length(orderToReportStims)
                            RF(b) = PAL_AMRF_updateRF(RF(b), -log10(aSigma(b)), feedback(orderToReportStims(j)));
                        end
