...
%   estParams         : Final estimated parameters [alpha, beta, gamma, lambda]
    ...
    if doPlot
        ...
        betas = linspace(0.05, 1, plotGrain);
        gammas = gamma;
    ...
    %%%%%%% Algorithm Setup
    priorAlphaRange = linspace(-70, -30, 50); 
    priorBetaRange  = linspace(0.05, 1, 50);
    ...
    prior = ones(size(PM.priorAlphas));%prior = PAL_pdfNormal(PM.priorAlphas, -50, 20);
    ...
    while PM.stop ~= 1
        ...
        PM = PAL_AMPM_updatePM1(PM,response,'fixLapse',suspend);
    ...
    pdf = PM.pdf ./ sum(PM.pdf(:));
    estAlpha = sum(PM.priorAlphas(:) .* pdf(:));
    estBeta = sum(PM.priorBetas(:) .* pdf(:));
    estLambda = sum(PM.priorLambdas(:) .* pdf(:));

%     [~, MAPindex] = PAL_findMax(PM.pdf);
%     MAPalpha = priorAlphaRange(MAPindex(1));
%     MAPbeta  = priorBetaRange(MAPindex(2));
%     MAPlambda = priorLambdaRange(MAPindex(4));
%     PM.threshold(end) = MAPalpha;
%     PM.slope(end) = MAPbeta;
%     PM.lapse(end) = MAPlambda;
    estParams = [estAlpha, estBeta, PM.guess(end), estLambda];
