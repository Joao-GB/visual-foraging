function [fixCenter, stimCenters] = getStimLocations3(ROIparams, nStims, minDist, ~, ~)
    % Poisson mais regular: sorteia dois para cada ponto
    ...
    while ~isempty(activeList) && size(allPoints,1) < (nStims + 1)
        ...
        for i = 1:k
            ...
            if ~collision
                phi = 2*pi*rand;
                cand2 = cand + minDist * [cos(phi), sin(phi)];
                
                if inEllipse(cand2(1), cand2(2))
                    
                    % check collisions for cand2 as well (same grid logic)
                    
                    if ~collision2
                        allPoints = [allPoints; cand; cand2];
                        activeList = [activeList; cand; cand2];
                        
                        % register both in grid
                    end
                end
