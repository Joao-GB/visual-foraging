...
function [tkP, savedNdone, fixCenters, stimCenters, orientation] = runForaging(tkP, dpP, drP, txP, prm, debug, mode)
        ...
        mode = modeMap(mode);
        if mode == 2, suffix = 'trn'; else, suffix = 'exp'; end
    ...
    %% 5) Início Fase 1: tela de fixação
        ...
        %    cruz de fixação
                        ...
                        if ~fixAcquired && keepGoingTrials && ~restartTrial
                            if debug == 0 && mode >= 2
                                Eyelink('Command','record_status_message "TRIAL TIMEOUT PHASE 1"');
                            ...
                            while true
                                ...
                                if keyIsDown
                                    if keyCode(rKey)
                                        if debug == 0 && mode >= 2
                                            Eyelink('Command','record_status_message "TRIAL RECAL REQUESTED"');
                                            EyelinkDoTrackerSetup(tkP.el);
                                        else
    ...
    %% 6) Início Fase 2: tela de estímulos
            ...
            % vi. Registra o momento de início do trial
                        if debug == 0  && mode >= 2 && keepGoingTrials
                            Eyelink('Command', 'record_status_message "TRIAL ONSET PHASE 2"');
    ...
    %% 7) Início Fase 3: tela com ruído rosa
        ...
        %     medFixTime segundo antes de o ruído rosa substituí-lo
            ...
            % viii. Registra os tempos de início e fim da Fase 3
                        ...
                        if debug == 0 && mode >= 2
                            Eyelink('Command','record_status_message "TRIAL ONSET PHASE 3"');
%                             Eyelink('Message', sprintf('update_onset_%1d', updateStimOnset));
                            ...
                            Eyelink('Command','record_status_message "TRIAL OFFSET PHASE 3"');
    ...
    %% 8) Início Fase 4: reportar em quais posições havia alvos
            ...
            %    a identificação deles como visitados, atual e não visitados
                        ...
                        for auxIdx = 1:3
                         ...
                         if debug == 0  && mode >= 2
                            Eyelink('Command','record_status_message "TRIAL ONSET PHASE 4"');
%                             Eyelink('Message', sprintf('answer_onset_%1d', GetSecs));
                        ...
                        if debug == 0 && mode >= 2
                            Eyelink('Command','record_status_message "TRIAL OFFSET PHASE 4"');
...
%                         Eyelink('Message', sprintf('trial_offset_%1d', trialOffset));
                ...
                if b == tkP.nBlocks && keepGoingTrials
                    Eyelink('Command','record_status_message "SESSION OFFSET COMPLETED"');
                    blocksCompleted = true;
                else
                    Eyelink('Command','record_status_message "SESSION OFFSET INTERRUPTED"');
                end
            end
...
function [kgBlocks, rBlock, kgTrials, rTrial] = pauseHandle(kgBlocks, rBlock, kgTrials, rTrial, wasRecording, tkP, dpP, drP, prm, pauseMode, debug, foragingMode)
    pauseStart = GetSecs;
    Eyelink('Command','record_status_message "PAUSE ONSET"');
%     Eyelink('Message', 'PAUSE ONSET');
    ...
    if strcmp(pauseMode, 'trial')
        if strcmp(decision, 'gotoMenu')
            Eyelink('Command','record_status_message "PAUSE > MENU"');
%             Eyelink('Message', 'PAUSE > MENU');
            kgTrials = false;
            kgBlocks = false;
        elseif strcmp(decision, 'restartBlock')
            Eyelink('Command','record_status_message "PAUSE > RESTART"');
%             Eyelink('Message', 'PAUSE > RESTART');
            rBlock = true;
            kgTrials = false;
        elseif strcmp(decision, 'recalibrate')
            Eyelink('Command','record_status_message "PAUSE > RECAL"');
%           Eyelink('Message', 'PAUSE > RECAL');
        ...
        elseif strcmp(decision, 'resume')
            Eyelink('Command','record_status_message "PAUSE > RESUME"');
            rTrial = true;
        end
        pauseEnd = GetSecs;
...
%             Eyelink('Message', 'PAUSE_TO_MENU');
        ...
        elseif strcmp(decision, 'recalibrate')
            Eyelink('Command','record_status_message "PAUSE > RECAL"');
            if debug == 0 && mode >= 2
%                 Eyelink('Message', 'PAUSE_TO_RECALIBRATION');
                EyelinkDoTrackerSetup(tkP.el);
            else
                disp('Recalibragem solicitada')
            end
    ...
    Eyelink('Command','record_status_message "PAUSE OFFSET"');
end
