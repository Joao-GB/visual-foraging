function ForagingGabors(nTrials, nStims, nTargets)
% Versão (que pretende ser) final, com estímulo pré-sacádico em alguns
% trials e report das orientações
% A tarefa consiste em nTrials, cada um com nStims e quantidade média de
% nTargets, espalhados pseudoaleatoriamente na tela. A tarefa do sujeito é
% encontrar, dentre gabores de alta frequência com orientações variadas, 
% alvos com uma orientação específica, apenas com o movimento ocular. Em
% determinado momento, durante essa busca ocular, os estímulos ainda não
% visitados são substituídos por um ruído rosa de mesma orientação durante
% o período pré-sacádico (calculado com o tempo médio das fixações do 
% sujeito)
%
...
%% de cada participante, algum parâmetro do ruído rosa
        ...
        if nTargets > 1 && nTargets < nStims
            ...
            As = randsample(targetsSpace, nTrials, true, numTargetsPMF);
        else
...
%% 1) Parâmetros das condições experimentais
    ...
    % (e) Distribuição do instante de modificação dos demais estímulos
        ...
        auxNs = randsample(1:nStims, nTrials, true, targetModTimePMF);
        Ns = auxNs - 1;
    ...
    % (g) Quantidade de estímulos cuja orientação deve ser reportada
        ...
        nStimsToReport = round((maxToReport - minToReport)*rand(1,nTrials) + minToReport);
        reportPostModFix = rand(1, nTrials) < propTrialsPSA;
        nStimsToReport = nStimsToReport - reportPostModFix;
        auxBool = nStimsToReport >= 2;
        auxDistr = binornd(nStimsToReport-2*auxBool, 0.5);
        reportStimsFromSets = [auxDistr + auxBool;nStimsToReport - auxDistr - auxBool];
        nStimsToReport = [reportStimsFromSets(1,:); reportPostModFix; reportStimsFromSets(2,:)];
...
%         correctnSTR3 = nStims - auxNs <= nStimsToReport(3,:);
        ...
        [~, orderToReportSets] = sort(rand(3, nTrials), 1);
...
%% 3) Qualidades dos Gabores
    ...
    %     quantidade aleatória de estímulos diagonais e cardinais
        ...
        %% Ajustar conforme o participante
        allOri(allOri ==targetOri) = [];
        orientation = allOri(randi(length(allOri), nTrials, nStims));
...
%% 7) Localização dos estímulos (pré-computada para todos trials)
    ...
    % (b) Atribui orientação não nula para apenas A(i) dos Gabores
            orientation(i, randperm(nStims, As(i))) = targetOri;
        end
...
%% 9) Desenha os Gabores em cada trial
    ...
    try
        ...
        %    a identificação deles como visitados, atual e não visitados
            ...
            seenAux = datasample(seenIdx, min(length(seenIdx), nStimsToReport(1, i)), 'Replace', false);
            currAux = []; if nStimsToReport(2, i) == 1, currAux = currIdx; end
            notSeenAux = datasample(notSeenIdx, min(length(notSeenIdx), nStimsToReport(3, i)), 'Replace', false);
            orderToReportStimsCell = {seenAux, currAux, notSeenAux};
            orderToReportStims = [orderToReportStimsCell{orderToReportSets(1)} orderToReportStimsCell{orderToReportSets(2)} orderToReportStimsCell{orderToReportSets(3)}];
